...
function returndesk_load_textdomain(): void {
	load_plugin_textdomain( 'windcodex-returndesk', false, dirname( RETURNDESK_PLUGIN_BASE ) . '/languages' );
}
add_action( 'init', 'returndesk_load_textdomain' );
...
function returndesk_check_pro_active(): void {
	if ( defined( 'RETURNDESK_PRO_VERSION' ) ) {
		add_action( 'admin_notices', 'returndesk_pro_active_notice' );
		if ( function_exists( 'deactivate_plugins' ) ) {
			deactivate_plugins( RETURNDESK_PLUGIN_BASE );
		}
	}
}
add_action( 'plugins_loaded', 'returndesk_check_pro_active' );

function returndesk_pro_active_notice(): void {
	echo '<div class="notice notice-error"><p>';
	echo wp_kses_post(
		sprintf(
			/* translators: %s: ReturnDesk Pro plugin URL */
			__( '<strong>WindCodex ReturnDesk</strong> cannot be activated while <strong>ReturnDesk Pro</strong> is active.', 'windcodex-returndesk' ),
			'https://www.windcodex.com/returndesk/'
		)
	);
	echo '</p></div>';
}
...
function returndesk_init(): void {
	...
	require_once RETURNDESK_PLUGIN_DIR . 'admin/class-returndesk-notices.php';
...
function returndesk_activate(): void {
	...
	if ( ! get_option( 'returndesk_activated_time' ) ) {
